  ...
  for (let day = 1; day <= totalDays; day++) {
    ...
    props.users.forEach(user => {
      if (user.birthdate) {
        ...
        if (bDate.getMonth() === currentMonth.value && bDate.getDate() === day) {
          events.push({
            type: 'birthday',
...
<template>
  <div class="calendar">
    <div class="calendar-header">
      <v-btn icon="mdi-chevron-left" @click="prevMonth"></v-btn>
      ...
      >
        ...
        <div class="day-events">
          <v-chip
            ...
            size="x-small"
            class="mt-1"
            :prepend-icon="event.type === 'birthday' ? 'mdi-cake-variant' : 'mdi-star'"
          >
            {{ event.title }}
          </v-chip>
        </div>
