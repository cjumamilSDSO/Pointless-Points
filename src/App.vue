...
const addPoints = (userId, points) => {
  ...
  if (user) {
    user.points += points;
    saveUsers();
    addAuditLog('points', `Awarded ${points} point${points > 1 ? 's' : ''} to ${user.name}`);
  }
};
...
const saveImportantDates = () => {
  localStorage.setItem('roll-call-important-dates', JSON.stringify(importantDates.value));
};
...
<template>
  <v-app>
    <v-app-bar color="primary">
      ...
      <v-toolbar-title>Team Roll Call Points</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn
        ...
        <v-row>
          ...
          <v-col cols="12" md="4">
            ...
            <v-card v-if="isSupervisor">
              ...
              <v-card-text>
                <PointsManager :users="users" @add-points="addPoints" />
              </v-card-text>
            </v-card>
          </v-col>
        </v-row>
    ...
    <v-footer app color="primary" class="text-center d-flex justify-center">
      <span class="text-white">Team Roll Call Points App - {{ new Date().getFullYear() }}</span>
    </v-footer>
  </v-app>
</template>
